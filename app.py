...
with st.sidebar:
    ...
    if st.button("Sinyal Gönder (Test)"):
        ...
        try:
            ...
            if res.status_code == 200: st.success("Gönderildi!")
            else: st.error("Hata!")
...
with tab1:
    ...
    if not df.empty:
        ...
        with c_right:
            ...
            last = df.iloc[0]
            st.markdown(f"""
            <div style="background-color:#161b22; padding:15px; border-radius:12px; border:1px solid #f39c12">
                <h3 style="color:#f39c12; margin-bottom:5px;">{last['pattern']}</h3>
                <p style="margin:0;"><b>Yön:</b> {'BUY' if last['signal']=='BUY' else 'SELL'}</p>
                <p style="margin:0;"><b>Giriş:</b> {last['entry']}</p>
                <p style="margin:0;"><b>Stop:</b> {last['sl']}</p>
                <p style="margin:0;"><b>TP2:</b> {last['tp2']}</p>
            </div>
            """, unsafe_allow_html=True)
...
with tab2:
    ...
    if daily_pnl <= -3000:
        st.markdown(f'<div style="background-color:#ff4b4b; color:white; padding:10px; border-radius:8px; text-align:center; font-weight:bold;">🚨 KRİTİK KAYIP: ${daily_pnl:.2f}</div>', unsafe_allow_html=True)
    elif daily_pnl >= 1000:
        st.markdown(f'<div style="background-color:#00c853; color:white; padding:10px; border-radius:8px; text-align:center; font-weight:bold;">✅ HEDEF TAMAM: ${daily_pnl:.2f}</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div style="background-color:#161b22; color:white; padding:10px; border-radius:8px; text-align:center;">Günlük: ${daily_pnl:.2f}</div>', unsafe_allow_html=True)

    k1, k2, k3, k4 = st.columns(4)
    k1.metric("BAŞLANGIÇ", "$100k")
    k2.metric("KASA", f"${balance:,.2f}")
    k3.metric("T. KAR/ZARAR", f"${total_p:,.2f}")
